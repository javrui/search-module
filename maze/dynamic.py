...
LAYOUT_ELEMENTS = {
    ...
    'exploration':{'char':'·', 'color':208, 'wait':0.5},
    'solution':{'char':'¤', 'color':118, 'wait':0.5},
    'start_goal':{'char':'_', 'color':7, 'wait':0.5},
    }
...
def calculate_maze_and_solution_layout_elements(maze):
    ...
    # Elements in a dict
    maze_layout_elements = {'walls': maze_walls,
                            'start_goal': maze_start_goal,
                            'exploration': maze_exploration,
                            'solution': maze_solution,
                            }

    return maze_layout_elements


def show_maze_and_solution_layout_elements(stdscr, maze, maze_layout_elements):
    ...
    try:
        ...
        for element_name in ['walls', 'start_goal', 'exploration', 'start_goal','solution', 'start_goal']:
            for (x, y, char) in maze_layout_elements[element_name]:
...
def main(stdscr):
    ...
    maze_layout_elements = calculate_maze_and_solution_layout_elements(maze)
    show_maze_and_solution_layout_elements(stdscr, maze, maze_layout_elements)
